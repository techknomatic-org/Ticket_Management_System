...
const config = {
    entry: function () {
        ...
        const entries = {};
        const bundleDir = path.resolve(__dirname, 'lib/bundles');

        for (const ext of ['.js', '.scss']) {
            const files = fs.readdirSync(bundleDir).filter(f => f.endsWith(ext) && !f.endsWith('.min' + ext));
            for (const file of files) {
                const entry_name = path.basename(file, ext);
                if (entry_name in entries) {
                    throw new Error(`Duplicate bundle entry: '${entry_name}'.`);
                }
                entries[entry_name] = './' + path.posix.join('lib/bundles', file);
            }
        }
