...
final class Server
{
    ...
    private static function doGenerateKeys(): void
    {
        ...
        // On Windows, OpenSSL requires an openssl.cnf configuration path
        $openssl_conf = getenv('OPENSSL_CONF');
        if (empty($openssl_conf) || !file_exists($openssl_conf)) {
            $php_dir = defined('PHP_BINARY') && !empty(PHP_BINARY) ? dirname(PHP_BINARY) : '';
            $candidates = [
                $php_dir !== '' ? $php_dir . DIRECTORY_SEPARATOR . 'extras' . DIRECTORY_SEPARATOR . 'ssl' . DIRECTORY_SEPARATOR . 'openssl.cnf' : '',
                $php_dir !== '' ? $php_dir . DIRECTORY_SEPARATOR . 'openssl.cnf' : '',
                'C:\Program Files\Common Files\SSL\openssl.cnf',
                'C:\tools\php\extras\ssl\openssl.cnf',
        ...
        try {
            openssl_pkey_export_to_file($key, self::PRIVATE_KEY_PATH, null, $config);
        } catch (OpensslException $e) {
